...
**Status of this document:** describes the structure as of the 2026-09-22 architecture refactor (`ROBOTX_PI_ARCHITECTURE_REFACTOR_PLAN.md`). It documents what exists on disk today, distinguishing IMPLEMENTED / PARTIALLY IMPLEMENTED / EXPERIMENTAL / NOT IMPLEMENTED where relevant. It does not describe a fleet-management platform, a simulation environment, or any backend server — none of those exist in this repository. See `ROBOTX_DEEP_CURRENT_STATE_AUDIT.md` for the full line-by-line audit this structure was derived from, and `ROBOTX_PI_REMEDIATION_PLAN.md` for the safety/security remediation history (R-00 through R-10).
...
```
                    ┌─────────────────────────────────────────────┐
  HTTP  ───────────►│  robotx.application.app  (FastAPI, uvicorn)  │
...
tests/hardware/      Standalone hardware-exercise scripts (NOT automated/CI-safe)
...
| `vision_controller.py` | `VisionController` | **[EXPERIMENTAL — NOT WIRED INTO PRODUCTION]**. Continuous-turning, hysteresis-based obstacle avoidance with motion-depth trend estimation. Imports only other `perception.*` modules — never `hardware` or `control`. Exercised exclusively by `tests/hardware/test_vision.py`. Moved into `perception/` in this refactor because its dependencies were always perception-only; this is a structural relocation, **not** a promotion — it is still not called by `RobotController` |
...
`robot_controller.py` — `RobotController` is the **only** production control loop, wired into `robotx.application.app` at startup. Responsibilities:
...
- Builds the telemetry dict and forwards it to whatever hook `robotx.application.app` registered (decoupled from the transport).
...
- Inbound `command`/`manual` events are queued (`asyncio.Queue`, FIFO, no dedup) and dispatched one at a time to whatever handler `robotx.application.app` registered (`RobotController.handle_command`).
...
`app.py` is the composition root and sole entry point:
...
**[IMPLEMENTED]** `tests/hardware/` — six standalone scripts, each directly exercising real GPIO/camera/serial hardware. None are automated (no `pytest`, no assertions) and none should ever be run as part of an automated check, CI, or this kind of refactor validation, because every one of them can move motors or requires a human to observe the robot.

| Script | Touches |
|---|---|
| `test_motor.py` | Real motors (drives forward/backward) |
| `test_gps.py` | Real serial GPS |
| `test_ultrasonic.py` | Real HC-SR04 |
| `test_controller.py` | Real motors + ultrasonic + IR + encoders (independent reimplementation — does **not** exercise `RobotController`; misleadingly named, tracked as R-05, not renamed by this refactor) |
| `test_cv.py` | Real Picamera2 |
| `test_vision.py` | Real Picamera2 + `VisionController`/`DecisionEngine` (the only exerciser of the experimental pipeline) |

**[NOT IMPLEMENTED]** `tests/unit/`, `tests/integration/` — no automated, hardware-free test suite exists yet for the pure-logic code (`haversine_m`, `RoutePlanner`, `decode_polyline`, `ObjectTracker`, `DecisionEngine`). This is tracked as R-05/future work in `ROBOTX_PI_REMEDIATION_PLAN.md`, not created by this structural refactor (empty test directories were deliberately not scaffolded — see the refactor plan's over-engineering guardrails).
...
```bash
venv/bin/python tests/hardware/test_motor.py       # SAFETY: wheels off the ground
venv/bin/python tests/hardware/test_ultrasonic.py
venv/bin/python tests/hardware/test_gps.py
venv/bin/python tests/hardware/test_controller.py  # SAFETY: wheels off the ground
venv/bin/python tests/hardware/test_cv.py
venv/bin/python tests/hardware/test_vision.py
```
